// =============================================================================
// KART LEGENDS: TITANIUM MASTER FINAL V4 (STABLE & RESTORED)
// CORREÇÃO: PISTA VISÍVEL, BOTS OFFLINE E PREVENÇÃO DE CRASH
// =============================================================================
...
(function() {
    ...
    const Logic = {
        state: 'MODE_SELECT',
        raceState: 'LOBBY',
        roomId: 'kart_legends_restored_v9',
        selectedChar: 0,
        selectedTrack: 0,
        isReady: false,
        isOnline: false,
            ...
            if (d.pos >= trackLength) { 
                if (d.maxLapPos > trackLength * 0.70) {
                    ...
                    if (d.lap > CONF.TOTAL_LAPS) {
                        ...
                        d.speed = 0;
                        window.Sfx.play(1000, 'sine', 1, 0.5); d.pushMsg("FINALIZADO!", "#ff0", 80); nitroBtn.style.display = 'none';
                        if(this.isOnline) this.syncMultiplayer();
                        KartAudio.stop(); 
                    } else { window.Sfx.play(700, 'square', 0.3, 0.1); d.pushMsg(`VOLTA ${d.lap}/${CONF.TOTAL_LAPS}`, "#fff", 60); }
                } else { d.pos = trackLength - 1; d.speed = 0; d.pushMsg("SENTIDO ERRADO!", "#f00"); }
            }
        ...
        renderWorld: function(ctx, w, h) {
            ...
            for(let n = CONF.DRAW_DISTANCE - 1; n >= 0; n--) {
                ...
                d.rivals.forEach(r => {
                    ...
                    if (Math.abs(Math.floor(relPos / CONF.SEGMENT_LENGTH) - n) < 2.0 && n > 0) {
                        this.drawKartSprite(ctx, coord.x + (r.x * (w*1.5) * coord.scale), coord.y, w*0.0055*coord.scale, 0, 0, 0, r.color, r.charId, true);
            ...
            // CORREÇÃO CRÍTICA: BLINDAGEM DO RAIO DE PARTÍCULAS
            particles.forEach(p => {
                if (p.l <= 0) return;
                ctx.fillStyle = p.c; 
                ctx.globalAlpha = Math.max(0, Math.min(1, p.l / p.maxL));
                const radius = Math.max(0.1, 4 + (p.maxL - p.l) * 0.5); // Garante raio positivo
                ctx.beginPath(); ctx.arc(p.x, p.y, radius, 0, Math.PI*2); ctx.fill();
            }); 
            ctx.globalAlpha = 1;

            if (d.state !== 'SPECTATE') {
                this.drawKartSprite(ctx, cx, h*0.85 + d.bounce, w * 0.0055, d.steer, d.visualTilt, d.spinAngle, CHARACTERS[d.selectedChar].color, d.selectedChar, false);
            }
        },
        ...
        renderUI: function(ctx, w, h) {
            ...
            if (d.virtualWheel.opacity > 0.05) {
                ...
                ctx.rotate(d.steer * 1.5); ctx.strokeStyle = '#fff'; ctx.lineWidth = 6;
                ctx.beginPath(); ctx.arc(0,0, d.virtualWheel.r, 0, 7); ctx.stroke();
                ctx.fillStyle = '#f00'; ctx.fillRect(-5, -d.virtualWheel.r, 10, 15);
                ctx.restore();
            }
        },
